...
\end{Verbatim}
		...
		\verb|cabstract| 和 \verb|eabstract| 环境用于编写中英文摘要。
		用户只需要写摘要的正文；标题、作者、导师、专业等部分会自动生成。

		\verb|\specialchap| 命令用于开始不进行标号但计入目录的一章，
		并合理安排其页眉。%
		\emph{%
			注意：
			在此章内的节或小节等命令应使用带星号的版本，
		...
		pkuthss 文档类建立在 ctexbook\cite{ctex-doc} 文档类的基础上，
		并调用了 CJKfntef、%
		graphicx\cite{graphicx-doc}、geometry\cite{geometry-doc} %
		和 fancyhdr\cite{fancyhdr-doc} 等几个宏包。
...
\end{Verbatim}
		...
		\begin{itemize}
			\item 启用 \verb|spacing| 选项时会调用 %
				setspace 和 footmisc\cite{footmisc-doc} 宏包。
			\item 启用 \verb|tightlist| 选项时会调用 %
				enumitem\cite{enumitem-doc} 和 %
				natbib\cite{natbib-doc} 宏包。
			\item 启用 \verb|tocbibind| 选项时会调用 %
				tocbibind\cite{tocbibind-doc} 宏包。
		...
		例如在本文档中的以下代码
\begin{Verbatim}[frame=single]
文献 \citenum{ctex-doc} 中提到
\end{Verbatim}
		将产生“文献 \citenum{ctex-doc} 中提到”。
